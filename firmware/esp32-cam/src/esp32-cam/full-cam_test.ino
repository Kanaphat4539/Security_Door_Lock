...
const char *SERVER_BASE = "http://192.168.137.1:3001";
...
Arduino_DataBus *bus =
    new Arduino_ESP32SPI(TFT_DC, TFT_CS, TFT_SCLK, TFT_MOSI, -1);

Arduino_GFX *gfx =
    new Arduino_ST7735(bus, TFT_RST, 1, /* rotation: 1 = แนวนอน (160x128) */
                       false,           /* ips: false */
                       128,             /* width */
                       160,             /* height */
                       0, 0, 0, 0);
...
// ==================== ESP32-CAM เป็นตัวที่ร้อนสุดในระบบ (WiFi + กล้องสตรีมต่อเนื่อง)
// อ่านอุณหภูมิชิปในตัว แล้วลด FPS ลงเมื่อร้อนเกิน เพื่อกันชิปร้อนจนภาพเพี้ยน/รีเซ็ตเอง
// และประหยัดพลังงาน หมายเหตุ: temperatureRead() วัด "อุณหภูมิแกนชิป" ไม่ใช่บรรยากาศ
// ปกติจึงสูงกว่าห้อง ~15-25°C
...
void manageThermal() {
  ...
  if (gChipTempC >= TEMP_HOT_C) {
    newInterval = FPS_HOT_MS;
    level = "HOT";
  } else if (gChipTempC >= TEMP_WARN_C) {
    newInterval = FPS_WARN_MS;
    level = "WARM";
  } else {
    newInterval = FPS_NORMAL_MS;
    level = "OK";
  }
