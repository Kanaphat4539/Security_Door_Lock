...
SRC = os.path.normpath(os.path.join(HERE, "..", "mainboard"))
...
def maze_route(grid, net, start, goal, start_layers=(0, 1),
    ...
    def via_ok(ix, iy):
        # via radius (0.3) + clearance (0.2) ~= 2 cells at 0.25mm grid
        for dy in range(-2, 3):
            for dx in range(-2, 3):
                nx, ny = ix + dx, iy + dy
...
if __name__ == "__main__":
    ...
    for i, (net, vx, vy) in enumerate(vias_pts):
        key = (net, round(vx, 3), round(vy, 3))
        if key in seen_via:      # same net vias at one spot -> one hole
            continue
        seen_via.add(key)
